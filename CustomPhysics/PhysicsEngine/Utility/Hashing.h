...
namespace Hashing
{
	static constexpr const uint32_t k_crc_table[256] =
	{
	...
	constexpr uint32_t compute(const char* data, uint32_t len, uint32_t crc = 0)
	{
		...
		{
			crc = k_crc_table[*data ^ (crc & 0xFF)] ^ (crc >> 8);
			data++;
		}
		crc = crc ^ 0xFFFFFFFFU;
		return crc;
...
#define CRC32_STR(A) std::integral_constant<uint32_t, Hashing::compute(A, sizeof(A)-1)>::value
